...
class CodeGenerator(object):
    ...
    def call2str(self, call: nodes.Call, as_stmt=False, **_):
        """Возвращает вызов функции или метода в виде строки."""
        ...
        if callable(name):
            name = return_namedtuple('CallType', 'name', 'args', 'kwargs', 'body')(name)
            func = name(self.get_function_name(call), *args, **kwargs)
            print('Body', func.body)
            if func.body is not None:

                if not as_stmt:
                    raise Error('It is not possible to convert a function call to a string if the function is converted to a block tag.')

                open_tag = self.get_block_stmt(
                    self.make_call_function_string(func.name, func.args, func.kwargs, call.dyn_args, call.dyn_kwargs)
                )
                close_tag = self.get_block_stmt(f'end{func.name}')

                return f'{open_tag}{func.body}{close_tag}'

            name, args, kwargs, _ = func

        result = self.make_call_function_string(name, args, kwargs, call.dyn_args, call.dyn_kwargs)

        if as_stmt:
            result = self.get_block_stmt(result)
    ...
    def get_function_name(self, call: nodes.Call, use_mapping=False):
        ...
        def f(name, mapper):
            ...
            for i, callback in mapper.items():
                if isinstance(i, re.Pattern) and i.match(name):
                    return callback
        ...
        return f(name, self.functions_mapper) or name
...
class DjangoCodeGenerator(CodeGenerator):
    ...
    def is_template_tag(self, call):
        ...
        # name = self.get_function_name(call, use_mapping=True)
        name = self.get_function_name(call)
    ...
    def output2str(self, output: nodes.Output):
        ...
        for node in output.nodes:
            if isinstance(node, nodes.Call) and self.is_template_tag(node):
                result.append(self.call2str(node, as_stmt=True))
